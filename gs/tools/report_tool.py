...
def download_all_excel(group,folder,date, qmail):
			...
			if qmail:
				...
				for filename, mail_msg, part in qmail.mail_list:
					if mail_msg:
						...
						key_list = new_filename.split('.')[0].split('-')
						if group[i]['name'].upper() == key_list[0].upper() and data['name'].upper() == key_list[
							1].upper() and list(map(int, date[0:3])) == list(map(int, key_list[2:5])):
							print(mail_subject)
							bo = qmail.download_file(folder, new_filename, part)
							if not bo:
								logger.error('下载附件失败')
								continue
							logger.info('下载附件成功')
							small_excel_path = check_small_excel_path(folder, group[i]['name'], data['name'],
																	  date)
...
def run():
	...
	if not config.get('user') or not config.get('password'):
		...
		qmail = ImapMail(config['user'], config['password'])
				...
				if qmail:
					...
					for mail_msg in qmail.mails:
						if mail_msg:
							try:
								p = re.match(r'.*({}'.format(
									f"{group[i]['name']}-{data['name']}") + r'-(\d\d\d\d)-(\d{1,2})-(\d{1,2})).*',
											 mail_msg.get('Subject'))
							except Exception as e:
								logger.exception(e)
								p = None
							if p:
								groups = p.groups()
								if int(date[1]) == int(groups[2]) and int(date[2]) == int(groups[3]) and int(
										date[0]) == int(groups[1]):
