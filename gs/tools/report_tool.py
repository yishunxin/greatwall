...
def run():
    ...
    if add_title:
        small_count += 1
        sheet_total.insert_rows(index, 1)
        titles = ['Sunday', 'Large', 'Medium', 'Small', 'SF', 'L3+', 'L2', 'L1', 'L0', 'Adults', 'Children',
                  'Attendance', 'Newcomers', 'Absence', 'Care', 'Lost Sheep', 'Cover', 'Absen 1', 'Absen 2', 'Absen 3',
                  'Absen 4', 'Absen 1', 'Absen 2', 'Absen 3', 'Absen 4', 'Abs Ttl']
    ...
    for i in range(len(group)):
        ...
        for j in range(len(group[i]['child'])):
            ...
            for a in range(1, ws_small.max_row):
                if ws_small.cell(a, 1).value == u'人数统计' and ws_small.cell(a + 1, 1).value == u'类别' and ws_small.cell(
                                a + 2, 1).value == u'颜色定义':
                    index_small = a
                    break
            adult_ab_list = []
...
def new_run():
    config, small_count = load_config()
    logger.info('load config:{}'.format(config))
    date = config.get('date')
    group = config.get('group')
    flist = os.listdir('./')
    for fname in flist:
        if fname.startswith('统计表-') and fname.endswith('.xlsx'):
            old_name = fname
            break
    logger.info('old_excel:%s' % old_name)
    export_name = '统计表-{}-{}-{}.xlsx'.format(date[0], date[1], date[2])
    folder = './{}-{}'.format(date[0], date[1])
    small_excels = os.listdir(folder)
    logger.info('small_excels:{}'.format(small_excels))
    small_excels_file_dict = {item: item.split('.')[0].split('-') for item in small_excels}
    # load 统计表
